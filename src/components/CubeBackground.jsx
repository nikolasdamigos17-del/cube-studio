...
export default function CubeBackground() {
  ...
  useEffect(() => {
    ...
    // Stagger: let React mount the new page and run its 0.32s fade first,
    // THEN start the cube spin. Running both at once was the main source
    // of route-change jank — they competed for the same frame budget.
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      const dpr = Math.min(window.devicePixelRatio || 1, coarse ? 1.0 : 1.5); // phones/tablets render at 1x
    ...
    let ambientCount = 0;
    let MAX_AMBIENT = 12; // fewer concurrent particles = fewer draw calls per frame
    ...
    let lastFrameTime = 0;
    let FRAME_INTERVAL = 1000 / 30; // cap at 30fps — smooth but lighter than 60fps
    // Adaptive quality: if the device consistently can't hit the frame budget,
    // permanently drop to a lighter mode (fewer particles, no corner beams,
    // 24fps). One-way switch — flickering between modes would look worse.
    ...
    const draw = (ts) => {
      ...
      if (lastFrameTime && (ts - lastFrameTime) > FRAME_INTERVAL * 1.9) {
        if (++slowFrames > 24 && !degraded) {
          degraded = true;
          MAX_AMBIENT = 6;
