...
import { ArrowLeft, Users2, ChevronRight, Dumbbell, Save } from 'lucide-react';
import { db } from '../lib/db';
import { groupDisplayName, groupWeek, hasNutrition } from '../lib/groups';
import { getGroupTrainingBalance } from '../lib/credits';
...
export default function GroupProfile() {
  ...
  const [form, setForm] = useState({ sessions_per_week:'' });
  ...
  const load = async () => {
    ...
    if (g) getGroupTrainingBalance(g).then(setTokens).catch(()=>setTokens(null));
  };
  ...
  const save = async () => {
    ...
    const patch = { sessions_per_week:w };
    ...
    <div className="p-6 md:p-8 max-w-3xl mx-auto animate-fade-in">
      ...
      <div className="flex items-center gap-4 mb-6 flex-wrap">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-gray-400">Ομαδικό πλάνο · {mem.length} μέλη · κοινά tokens προπονήσεων</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm mb-5">
        ...
        <div className="mt-4 flex items-center gap-3 flex-wrap text-sm">
          ...
          {tokens !== null && (
            <button onClick={()=>navigate('/Logistics')} className={`px-3 py-1.5 rounded-full font-semibold ${tokens<=0?'bg-red-50 text-red-600':tokens<=2?'bg-amber-50 text-amber-600':'bg-emerald-50 text-emerald-700'}`}>
              🎟 Υπόλοιπο group: {tokens} → Logistics
            </button>
          )}
        </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
            ...
            <button key={m.id} onClick={()=>navigate(`/ClientProfile?id=${m.id}`)} className="w-full flex items-center gap-3 p-3 rounded-xl border border-gray-100 hover:border-gray-900 text-left">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400">
                  🏋️ {weekNow}×/εβδ.
                  {hasNutrition(m) ? ` · 🥗 ${m.nutrition_meetings_per_month||0} διατροφές/μήνα` : ''}
                </p>
              </div>
        ...
        <p className="text-[11px] text-gray-400 mt-3">Για τη διατροφή κάθε μέλους, άνοιξε την καρτέλα του.</p>
      </div>
    </div>
  );
}
