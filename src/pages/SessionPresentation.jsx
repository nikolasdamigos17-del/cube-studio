...
export default function SessionPresentation({ client, allProgress, onBack }) {
  ...
  // ── Comparison picker: any two saved records (defaults: two most recent) ──
  const [cmpB, setCmpB] = useState(allProgress.length - 1);                       // "Now"
  const [cmpA, setCmpA] = useState(allProgress.length > 1 ? allProgress.length - 2 : -1); // "Prev"
  const latest = allProgress[cmpB];
  const prev   = cmpA >= 0 ? allProgress[cmpA] : null;
  ...
  const series = (key) => {
    const rows = allProgress.slice(0, cmpB + 1).filter(r=>r[key]!=null);
    return {
  ...
  const isMobileSP = typeof window !== 'undefined' && window.innerWidth < 768;
  const S = {
    page:  { minHeight:'100vh', background:C.bg, color:'#f0f0f8', fontFamily:'Inter,system-ui,sans-serif', display:'flex', flexDirection: isMobileSP?'column':'row' },
    aside: isMobileSP
      ? { position:'fixed', bottom:0, left:0, right:0, height:'calc(58px + env(safe-area-inset-bottom))', paddingBottom:'env(safe-area-inset-bottom)', background:'#04040b', borderTop:`1px solid ${C.border}`, display:'flex', flexDirection:'row', alignItems:'center', justifyContent:'space-around', zIndex:40 }
      : { width:64, flexShrink:0, background:'#04040b', borderRight:`1px solid ${C.border}`, display:'flex', flexDirection:'column', alignItems:'center', padding:'20px 0', gap:6, position:'fixed', top:0, left:0, height:'100vh', zIndex:20 },
    main:  { flex:1, marginLeft: isMobileSP?0:64, paddingBottom: isMobileSP?'70px':0, display:'flex', flexDirection:'column', minHeight:'100vh' },
  };
    ...
    <div style={S.page}>
      ...
      <aside style={S.aside}>
        <button onClick={onBack}
          style={{width:38,height:38,borderRadius:11,border:`1px solid ${C.border}`,background:'transparent',color:C.dim,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',marginBottom:isMobileSP?0:14,flexShrink:0}}>
      ...
      <main style={S.main}>
        ...
        <div style={{padding:'32px 32px 20px', borderBottom:`1px solid ${C.border}`}}>
          <div style={{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap',marginBottom:6}}>
            <p style={{fontSize:10,fontWeight:700,letterSpacing:'0.15em',textTransform:'uppercase',color:'rgba(255,255,255,0.22)',margin:0}}>
            ...
            {allProgress.length > 1 && (
              <div style={{display:'flex',alignItems:'center',gap:6}}>
                <select value={cmpA}
                  onChange={e=>setCmpA(parseInt(e.target.value))}
                  style={{background:'rgba(255,255,255,0.06)',border:`1px solid ${C.border}`,
                    borderRadius:8,color:'rgba(255,255,255,0.75)',fontSize:11,
                    padding:'4px 8px',cursor:'pointer',outline:'none'}}>
                  <option value={-1} style={{background:'#0a0a14'}}>— none —</option>
                  {allProgress.map((r,i)=>(
                    i!==cmpB && <option key={i} value={i} style={{background:'#0a0a14'}}>
                      {r.date?format(parseISO(r.date),'MMM d, yyyy'):`Record ${i+1}`}
                    </option>
                  ))}
                </select>
                <span style={{fontSize:11,color:'rgba(255,255,255,0.3)'}}>→</span>
                <select value={cmpB}
                  onChange={e=>{const v=parseInt(e.target.value); setCmpB(v); if(cmpA===v) setCmpA(-1);}}
                  style={{background:'rgba(129,140,248,0.12)',border:'1px solid rgba(129,140,248,0.4)',
                    borderRadius:8,color:'#a5b4fc',fontSize:11,fontWeight:600,
                    padding:'4px 8px',cursor:'pointer',outline:'none'}}>
                  {allProgress.map((r,i)=>(
                    <option key={i} value={i} style={{background:'#0a0a14'}}>
                      {r.date?format(parseISO(r.date),'MMM d, yyyy'):`Record ${i+1}`}
                    </option>
                  ))}
                </select>
